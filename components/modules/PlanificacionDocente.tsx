...
const ACTIVIDADES_CALENDARIO_KEY = 'actividadesCalendario';
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-fade-in-up" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-4 overflow-y-auto">
                    {Object.entries(editedLesson).map(([key, value]) => (
                        <div key={key}>
                            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 capitalize mb-1">{key.replace(/([A-Z])/g, ' $1')}</label>
                            <textarea
                                name={key}
                ...
                <div className="bg-slate-50 dark:bg-slate-700/50 px-6 py-4 rounded-b-xl flex justify-end gap-3 mt-auto">
                    <button onClick={onClose} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded-lg hover:bg-slate-300">Cancelar</button>
                    <button onClick={handleSubmit} className="bg-amber-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-amber-600">Guardar Cambios</button>
                </div>
            </div>
        </div>
    );
...
const LessonPlanViewer: React.FC<LessonPlanViewerProps> = ({ plan, onEditLesson, onUseLesson }) => {
    return (
            ...
            <div className="space-y-4">
                {plan.detallesLeccion.map((lesson, index) => (
                    <div key={index} className="p-4 border dark:border-slate-700 rounded-lg">
                        <h4 className="font-bold text-lg text-slate-800 dark:text-slate-200">Clase {index + 1}: {lesson.actividades}</h4>
                        ...
                        <div className="flex gap-2 mt-4">
                            <button onClick={() => onEditLesson(index, lesson)} className="text-sm font-semibold bg-slate-200 dark:bg-slate-600 py-1 px-3 rounded-md hover:bg-slate-300">Editar</button>
                            <button onClick={() => onUseLesson(lesson, plan)} className="text-sm font-semibold bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300 py-1 px-3 rounded-md hover:bg-blue-200">Usar para Plan de Clase</button>
                        </div>
                    </div>
                ))}
            </div>
...
const ClassPlanViewer: React.FC<ClassPlanViewerProps> = ({ plan, onBack, onSave }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-xl shadow-md">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={onBack} className="text-slate-600 hover:text-slate-900 font-semibold">&larr; Volver</button>
            </div>
            <div className="space-y-6">
                <div>
                    <label className="font-bold text-lg text-slate-700 dark:text-slate-300">Inicio</label>
                    <textarea name="momentosClase.inicio" value={editablePlan.momentosClase.inicio} onChange={handleChange} rows={4} className="w-full mt-2 p-2 border rounded-md bg-slate-50 dark:bg-slate-700"/>
                </div>
                 <div>
                    <label className="font-bold text-lg text-slate-700 dark:text-slate-300">Desarrollo</label>
                    <textarea name="momentosClase.desarrollo" value={editablePlan.momentosClase.desarrollo} onChange={handleChange} rows={8} className="w-full mt-2 p-2 border rounded-md bg-slate-50 dark:bg-slate-700"/>
                </div>
                 <div>
                    <label className="font-bold text-lg text-slate-700 dark:text-slate-300">Cierre</label>
                    <textarea name="momentosClase.cierre" value={editablePlan.momentosClase.cierre} onChange={handleChange} rows={4} className="w-full mt-2 p-2 border rounded-md bg-slate-50 dark:bg-slate-700"/>
                </div>
            </div>
            <div className="text-right mt-6">
                <button onClick={() => onSave(editablePlan)} className="bg-amber-500 text-white font-bold py-2 px-6 rounded-lg hover:bg-amber-600">Guardar Cambios</button>
            </div>
        </div>
    );
};
...
// --- SUBMÓDULO 3: ACTIVIDADES CALENDARIO ---
const ActividadesCalendarioSubmodule: React.FC = () => {
    ...
    useEffect(() => {
        const data = localStorage.getItem(ACTIVIDADES_CALENDARIO_KEY);
        if (data) setActividades(JSON.parse(data));
    }, []);
    ...
    useEffect(() => {
        if (view === 'summary' && currentActividad) {
            try {
                ...
                const currentEvents: CalendarEvent[] = calendarData ? JSON.parse(calendarData) : [];
                const eventExists = currentEvents.some(e => 
                    e.type === EventType.ACTIVIDAD_FOCALIZADA &&
                    e.date === currentActividad.fecha &&
                    (e as ActividadFocalizadaEvent).responsables === currentActividad.nombre &&
                    (e as ActividadFocalizadaEvent).ubicacion === currentActividad.ubicacion
                );
                setIsAddedToCalendar(eventExists);
            } catch (e) { console.error("Could not check calendar", e); }
        }
    }, [view, currentActividad]);

    const persistActividades = (data: ActividadPlanificada[]) => {
        setActividades(data);
        localStorage.setItem(ACTIVIDADES_CALENDARIO_KEY, JSON.stringify(data));
    };
    ...
    const handleSaveActivity = async (e: FormEvent) => {
        ...
        try {
            ...
            const newActividad: ActividadPlanificada = {
                id: crypto.randomUUID(),
                ...formData,
                objetivo: response.text.replace(/(\*\*|\*)/g, ''),
            };

            persistActividades([newActividad, ...actividades]);
            setCurrentActividad(newActividad);
            setView('summary');
        } catch (error) {
            ...
            const newActividad: ActividadPlanificada = { id: crypto.randomUUID(), ...formData, objetivo: 'No generado' };
            persistActividades([newActividad, ...actividades]);
            setCurrentActividad(newActividad);
            setView('summary');
        } finally {
    ...
    const handleAddToCalendar = (actividad: ActividadPlanificada) => {
        ...
        try {
            const calendarData = localStorage.getItem(CALENDAR_KEY);
            const currentEvents: CalendarEvent[] = calendarData ? JSON.parse(calendarData) : [];
            const updatedEvents = [{ ...newEvent, id: crypto.randomUUID() }, ...currentEvents];
            localStorage.setItem(CALENDAR_KEY, JSON.stringify(updatedEvents));
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-xl shadow-md">
                <form onSubmit={handleSaveActivity} className="space-y-8">
                    ...
                    <div className="p-4 border rounded-lg dark:border-slate-700">
                        ...
                        <div className="space-y-4">
                            <input name="nombre" value={formData.nombre} onChange={handleFormChange} placeholder="Nombre de la Actividad" className={inputStyles} required/>
                            <div className="grid grid-cols-2 gap-4">
                                <input name="fecha" type="date" value={formData.fecha} onChange={handleFormChange} className={inputStyles} required/>
                                <input name="hora" type="time" value={formData.hora} onChange={handleFormChange} className={inputStyles} required/>
                            </div>
                            <textarea name="descripcion" value={formData.descripcion} onChange={handleFormChange} placeholder="Descripción (objetivos, contenidos, metodología)" rows={4} className={inputStyles} required/>
                             <div className="grid grid-cols-2 gap-4">
                                <input name="ubicacion" value={formData.ubicacion} onChange={handleFormChange} placeholder="Ubicación" className={inputStyles} required/>
                                <input name="participantes" value={formData.participantes} onChange={handleFormChange} placeholder="Participantes" className={inputStyles} required/>
                            </div>
                            <textarea name="recursosGenerales" value={formData.recursosGenerales} onChange={handleFormChange} placeholder="Recursos Generales Necesarios" rows={2} className={inputStyles}/>
                        </div>
                    </div>
                     {/* Tareas */}
                    <div className="p-4 border rounded-lg dark:border-slate-700">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button type="button" onClick={handleAddTarea} className="bg-slate-200 dark:bg-slate-600 text-sm font-semibold px-3 py-1 rounded-md">+ Agregar Tarea</button>
                        </div>
                        <div className="space-y-4 max-h-60 overflow-y-auto">
                            {formData.tareas.map((tarea, index) => (
                                <div key={tarea.id} className="p-3 bg-slate-50 dark:bg-slate-700/50 rounded-md relative">
                                    <button type="button" onClick={() => handleRemoveTarea(tarea.id)} className="absolute top-1 right-1 text-red-500 text-lg">&times;</button>
                                    <textarea value={tarea.descripcion} onChange={e => handleTareaChange(tarea.id, 'descripcion', e.target.value)} placeholder={`Descripción de la Tarea ${index + 1}`} rows={2} className={inputStyles} required/>
                                    <div className="grid grid-cols-2 gap-2 mt-2">
                                        <input value={tarea.responsable} onChange={e => handleTareaChange(tarea.id, 'responsable', e.target.value)} placeholder="Responsable" className={inputStyles} required/>
                                        <input value={tarea.recursos} onChange={e => handleTareaChange(tarea.id, 'recursos', e.target.value)} placeholder="Recursos Necesarios" className={inputStyles} />
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="flex justify-end gap-4">
                        <button type="button" onClick={() => setView('list')} className="bg-slate-200 dark:bg-slate-600 font-semibold px-6 py-2 rounded-lg">Cancelar</button>
                        <button type="submit" disabled={isLoading} className="bg-blue-600 text-white font-bold px-6 py-2 rounded-lg flex items-center min-w-[150px] justify-center">
             ...
             <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-xl shadow-md space-y-6">
                ...
                <div className="border rounded-lg p-6 dark:border-slate-700">
                    ...
                    <div className="space-y-4 text-sm">
                        <div className="flex justify-between border-b pb-2 dark:border-slate-600"><strong className="text-slate-500">Nombre:</strong> <span>{currentActividad.nombre}</span></div>
                        <div className="flex justify-between border-b pb-2 dark:border-slate-600"><strong className="text-slate-500">Fecha y Hora:</strong> <span>{new Date(`${currentActividad.fecha}T${currentActividad.hora}`).toLocaleString('es-CL')}</span></div>
                        <div className="border-b pb-2 dark:border-slate-600"><p className="font-bold text-slate-500">Descripción:</p><p>{currentActividad.descripcion}</p></div>
                        <div className="flex justify-between border-b pb-2 dark:border-slate-600"><strong className="text-slate-500">Ubicación:</strong> <span>{currentActividad.ubicacion}</span></div>
                        <div className="flex justify-between border-b pb-2 dark:border-slate-600"><strong className="text-slate-500">Participantes:</strong> <span>{currentActividad.participantes}</span></div>
                        <div className="border-b pb-2 dark:border-slate-600"><p className="font-bold text-slate-500">Recursos Necesarios:</p><p>{currentActividad.recursosGenerales}</p></div>
                        <div className="border-b pb-2 dark:border-slate-600"><p className="font-bold text-slate-500">Objetivo (Generado por IA):</p><p>{currentActividad.objetivo}</p></div>
                    </div>
                 ...
                 <button onClick={() => { setView('list'); setCurrentActividad(null); }} className="w-full text-center font-semibold text-blue-600 hover:underline">
    ...
    return (
        <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-xl shadow-md">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => { setView('form'); setFormData(initialFormState); }} className="bg-amber-500 text-white font-bold py-2 px-5 rounded-lg hover:bg-amber-600">Planificar Actividad</button>
            </div>
                ...
                {actividades.length > 0 ? (
                    actividades.map(act => (
                        <div key={act.id} className="p-3 bg-slate-50 dark:bg-slate-700/50 rounded-md border dark:border-slate-700">
                             ...
                             <p className="text-sm text-slate-500 dark:text-slate-400">{new Date(act.fecha+'T12:00:00').toLocaleDateString('es-CL')} - {act.ubicacion}</p>
                        </div>
                    ))
                ) : (
...
const PlanificacionDocente: React.FC<PlanificacionDocenteProps> = ({ currentUser }) => {
    ...
    const persistPlanificaciones = useCallback((data: PlanificacionDocente[]) => {
        setPlanificaciones(data);
        localStorage.setItem(PLANIFICACIONES_KEY, JSON.stringify(data));
        window.dispatchEvent(new Event('storage'));
    }, []);

    useEffect(() => {
        try {
            const storedPlanificaciones = localStorage.getItem(PLANIFICACIONES_KEY);
            if (storedPlanificaciones) {
                setPlanificaciones(JSON.parse(storedPlanificaciones));
            }
        } catch (e) {
            console.error("Error al cargar planificaciones de localStorage", e);
        }
    }, []);
    ...
    const handleGenerateUnidad = async (e: FormEvent) => {
        ...
        try {
            ...
            const newPlan: PlanificacionUnidad = {
                id: editingPlanificacion?.id || crypto.randomUUID(),
            ...
            const updatedPlanificaciones = editingPlanificacion 
                ? planificaciones.map(p => p.id === editingPlanificacion.id ? newPlan : p)
                : [newPlan, ...planificaciones];

            persistPlanificaciones(updatedPlanificaciones);
            setEditingPlanificacion(newPlan);
    ...
    const handleUseLessonAsClassPlan = async (lessonDetail: DetalleLeccion, unitPlan: PlanificacionUnidad) => {
        ...
        try {
            ...
            const newClassPlan: PlanificacionClase = {
                id: crypto.randomUUID(),
                fechaCreacion: new Date().toISOString(),
                autor: currentUser.nombreCompleto,
                tipo: 'Clase',
            ...
            const allStoredPlans = JSON.parse(localStorage.getItem(PLANIFICACIONES_KEY) || '[]');
            const updatedPlanificaciones = [newClassPlan, ...allStoredPlans];
            persistPlanificaciones(updatedPlanificaciones);
    ...
    const handleDelete = (id: string) => {
        if (window.confirm("¿Está seguro de que desea eliminar esta planificación?")) {
            const updated = planificaciones.filter(p => p.id !== id);
    ...
    const handleSaveEditedLesson = (updatedLesson: DetalleLeccion) => {
        if (!editingLesson) return;
        const { planId, lessonIndex } = editingLesson;
        const updatedPlanificaciones = planificaciones.map(plan => {
            if (plan.id === planId && plan.tipo === 'Unidad') {
                const newDetallesLeccion = [...plan.detallesLeccion];
                newDetallesLeccion[lessonIndex] = updatedLesson;
                return { ...plan, detallesLeccion: newDetallesLeccion };
            }
            return plan;
        });
        persistPlanificaciones(updatedPlanificaciones);
        const updatedCurrentlyEditingPlan = updatedPlanificaciones.find(p => p.id === planId) as PlanificacionUnidad | undefined;
        if (updatedCurrentlyEditingPlan) {
            setEditingPlanificacion(updatedCurrentlyEditingPlan);
        }
    ...
    const renderUnidadTab = () => (
        <div className="space-y-8">
                ...
                <form onSubmit={handleGenerateUnidad} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div>
                            ...
                            <select name="asignatura" value={unidadFormData.asignatura} onChange={handleUnidadFieldChange} className={inputStyles}>
                        ...
                        <div>
                            ...
                            <select name="nivel" value={unidadFormData.nivel} onChange={handleUnidadFieldChange} className={inputStyles}>
                        ...
                        <div>
                            <label htmlFor="nombreUnidad" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">Nombre de Unidad <span className="text-red-500">*</span></label>
                            <input type="text" name="nombreUnidad" value={unidadFormData.nombreUnidad} onChange={handleUnidadFieldChange} className={inputStyles} placeholder="Ej: La Célula y sus procesos" />
                        </div>
                        <div className="md:col-span-2 lg:col-span-3">
                            <label htmlFor="contenidos" className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">Contenidos Clave <span className="text-red-500">*</span></label>
                            <textarea name="contenidos" value={unidadFormData.contenidos} onChange={handleUnidadFieldChange} rows={3} placeholder="Ingrese los temas, conceptos clave o unidades que la planificación debe abordar." className={inputStyles}></textarea>
                        </div>
                        <div className="md:col-span-2 lg:col-span-3">
                            ...
                            <textarea name="ideasParaUnidad" value={unidadFormData.ideasParaUnidad} onChange={handleUnidadFieldChange} rows={3} placeholder="Plantea tus ideas y perspectiva para que la IA construya una mejor planificación (ej: enfocar en trabajo colaborativo, usar casos prácticos, conectar con la actualidad)." className={inputStyles}></textarea>
                        </div>
                        <div>
                            ...
                            <input type="number" name="cantidadClases" value={unidadFormData.cantidadClases} onChange={handleUnidadFieldChange} className={inputStyles} min="1" max="20" />
                        </div>
                        <div className="md:col-span-2">
                            ...
                            <textarea name="observaciones" value={unidadFormData.observaciones} onChange={handleUnidadFieldChange} rows={1} placeholder="Ej: clase práctica, modelo ABP, juego, reforzamiento..." className={inputStyles}></textarea>
                        </div>
                    </div>

                    <div className="pt-4 text-right">
                        <button type="submit" disabled={loading} className="bg-slate-800 text-white font-bold py-2 px-6 rounded-lg hover:bg-slate-700 disabled:bg-slate-400 flex items-center justify-center min-w-[150px] dark:bg-amber-500 dark:text-slate-900 dark:hover:bg-amber-600">
                            {loading ? <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> : (editingPlanificacion ? 'Regenerar' : 'Generar Plan')}
                        </button>
                    </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-xl shadow-md">
                ...
                <div className="space-y-4">
                    {planificaciones.filter(p => p.tipo === 'Unidad').length > 0 ? planificaciones.filter((p): p is PlanificacionUnidad => p.tipo === 'Unidad').map(plan => (
                        <div key={plan.id} className={`p-4 border rounded-lg ${editingPlanificacion?.id === plan.id ? 'bg-amber-50 border-amber-300 dark:bg-amber-900/20 dark:border-amber-500/50' : 'bg-slate-50 dark:bg-slate-700/50 dark:border-slate-700'}`}>
                            <div className="flex justify-between items-center">
                                <div>
                                    <p className="font-bold text-slate-800 dark:text-slate-200">{plan.nombreUnidad}</p>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">Creado: {new Date(plan.fechaCreacion).toLocaleString('es-CL')}</p>
                                    <p className="text-sm text-slate-500 dark:text-slate-400 truncate max-w-md">Contenidos: {plan.contenidos}</p>
                                </div>
                                <div className="flex items-center gap-2">
                                     <button onClick={() => handleSelectForEdit(plan)} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-semibold text-sm">Ver y Editar</button>
    ...
    // Renders the list of generated class plans
    const renderClaseTab = () => {
        const planesClase = planificaciones.filter((p): p is PlanificacionClase => p.tipo === 'Clase').sort((a,b) => new Date(b.fechaCreacion).getTime() - new Date(a.fechaCreacion).getTime());
        ...
        if (viewingClassPlan) {
            return (
                <ClassPlanViewer 
                    ...
                    onSave={(updatedPlan) => {
                        const updatedPlanificaciones = planificaciones.map(p => p.id === updatedPlan.id ? updatedPlan : p);
                        persistPlanificaciones(updatedPlanificaciones);
                 ...
                 <div className="space-y-4">
                    {planesClase.length > 0 ? planesClase.map(plan => (
                        <div key={plan.id} className="p-4 border dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-700/50 flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                 <button onClick={() => setViewingClassPlan(plan)} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-semibold text-sm">Ver Plan de Clase</button>
                            </div>
                        </div>
                    )) : <p className="text-slate-500 dark:text-slate-400 text-center py-6">No hay planes de clase guardados. Puede generarlos desde una unidad en la pestaña 'Unidad'.</p>}
                </div>
            </div>
        );
    };
        ...
        <div className="space-y-8 animate-fade-in">
             ...
             <div className="border-b border-slate-200 dark:border-slate-700">
                <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                    <button onClick={() => setActiveTab('unidad')} className={`${activeTab === 'unidad' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:border-slate-600'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors focus:outline-none`}>
                        Unidad
                    </button>
                    <button onClick={() => setActiveTab('clase')} className={`${activeTab === 'clase' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:border-slate-600'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors focus:outline-none`}>
                        Clase
                    </button>
                     <button onClick={() => setActiveTab('calendario')} className={`${activeTab === 'calendario' ? 'border-amber-500 text-amber-600 dark:text-amber-400' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:border-slate-600'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors focus:outline-none`}>
            ...
            {activeTab === 'calendario' && <ActividadesCalendarioSubmodule />}
