...
const Administracion: React.FC = () => {
    ...
    useEffect(() => {
        try {
            ...
            setUserError("No se pudieron cargar los datos guardados.");
        }
    }, []);
    ...
    const handleUserSubmit = useCallback((e: FormEvent) => {
        e.preventDefault();
        ...
        if (editingUserId) {
            ...
            const updatedUser: User = {
                ...originalUser!,
                ...userFormData,
                password: finalPassword || originalUser?.password, // Keep old password if new one is blank
                curso: finalCurso,
                cursos: finalCursos,
        ...
        handleUserResetForm();
    }, [userFormData, editingUserId, users, handleUserResetForm]);
    ...
    const handleMassPasswordReset = () => {
        ...
        if (confirmation) {
            const updatedUsers = users.map(user => {
                ...
                return rest; // Remove the password field entirely
            });
            persistUsers(updatedUsers);
    ...
    const handleStudentBulkUpload = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (newStudents.length > 0) {
                    persistUsers([...newStudents, ...users]);
                }
    ...
    const handleStaffBulkUpload = (event: ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (!file) return;
