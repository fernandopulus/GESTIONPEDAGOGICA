...
// =========================
// Card de formulario reutilizable
// =========================
...
// =========================
// Cronograma (tabla estilo Gantt simple)
// =========================
const ProjectTimeline: React.FC<{
  ...
  const AssigneeAvatar: React.FC<{ name: string }> = ({ name }) => {
    ...
    const hashCode = (str: string) => { let hash = 0; for (let i = 0; i < str.length; i++) hash = str.charCodeAt(i) + ((hash << 5) - hash); return hash; };
    const colors = [
    ...
    return <div className={`w-10 h-10 rounded-full ${colors[colorIndex]} flex items-center justify-center font-semibold text-sm ring-2 ring-white dark:ring-slate-800 shadow-sm`}>{initials.slice(0, 2)}</div>;
  };

  const ActivityModal: React.FC<{ data: { planId: string; activity: ActividadInterdisciplinaria }; onClose: () => void; onSave: (planId: string, activity: ActividadInterdisciplinaria) => void; }> =
    ({ data, onClose, onSave }) => {
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl p-8 animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="space-y-6">
              <div>
                <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2"><FileText className="w-4 h-4" />Nombre de la Actividad</label>
                <input name="nombre" value={formData.nombre} onChange={handleChange} className="w-full p-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" placeholder="Ingrese el nombre de la actividad..." />
              </div>
              <div>
                <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2"><Users className="w-4 h-4" />Responsables</label>
                <input name="responsables" value={formData.responsables} onChange={handleChange} className="w-full p-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" placeholder="Responsables de la actividad..." />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2"><Calendar className="w-4 h-4" />Fecha de Inicio</label>
                  <input type="date" name="fechaInicio" value={formData.fechaInicio} onChange={handleChange} className="w-full p-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" />
                </div>
                <div>
                  <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2"><Calendar className="w-4 h-4" />Fecha de Fin</label>
                  <input type="date" name="fechaFin" value={formData.fechaFin} onChange={handleChange} className="w-full p-3 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" />
                </div>
              </div>
            </div>
            <div className="flex justify-end gap-4 mt-8">
              <button onClick={onClose} className="px-6 py-3 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg font-semibold hover:bg-slate-300 dark:hover:bg-slate-500 transition-colors">Cancelar</button>
              <button onClick={handleSave} className="px-6 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-colors shadow-lg">Guardar Cambios</button>
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-3 bg-slate-100 dark:bg-slate-700 rounded-xl"><Clock className="w-8 h-8 text-slate-600 dark:text-slate-400" /></div>
          <div>
        ...
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {allItems.map((item) => {
              ...
              return (
                <tr key={item.id} className={`${isClickable ? 'hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer transition-colors' : ''}`} onClick={() => isClickable && setEditingActivity({ planId: item.planId, activity: item })}>
                  <td className="px-6 py-4 text-base font-semibold text-slate-900 dark:text-slate-200">{item.nombre}</td>
                  <td className="px-6 py-4"><span className={`px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full ${status.color}`}>{status.text}</span></td>
    ...
    planificaciones.forEach((plan) => {
      plan.contenidosPorAsignatura?.forEach((contenido) => {
        contenido.habilidades?.forEach((habilidad) => {
          ...
          Object.entries(BLOOM_TAXONOMY).forEach(([categoria, hs]) => { if (hs.includes(habilidad)) categoriaCount[categoria] = (categoriaCount[categoria] || 0) + 1; });
        });
      });
    });
                ...
                <div className="space-y-4">
                  ...
                  {entrega.enlaceUrl && (
                    <div>
                      ...
                      <a href={entrega.enlaceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-sm font-medium">
                        <FileText className="w-4 h-4" /> {entrega.enlaceUrl}
                      </a>
                    </div>
                  )}
...
const PlanificacionForm: React.FC<{
  ...
  const handleAIGeneration = async (targetField: 'objetivos' | 'indicadoresLogro') => {
    if (!formData.descripcionProyecto.trim()) { alert('Por favor, ingrese una descripción del proyecto para usar la IA.'); return; }
    if (targetField === 'objetivos') setIsGeneratingObjectives(true); else setIsGeneratingIndicators(true);
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
    if (!apiKey) { alert('La API Key de Gemini no está configurada.'); setIsGeneratingObjectives(false); setIsGeneratingIndicators(false); return; }
    const prompt = targetField === 'objetivos'
      ? `Basado en la descripción de un proyecto escolar interdisciplinario, genera 3 a 5 objetivos de aprendizaje claros y medibles.
Descripción: "${formData.descripcionProyecto}"
Asignaturas involucradas: "${formData.asignaturas.join(', ')}"
Formato: lista con guiones.`
      : `Basado en la descripción y objetivos de un proyecto escolar, genera 5 a 7 indicadores de logro observables.
Descripción: "${formData.descripcionProyecto}"
Objetivos: "${formData.objetivos || 'Genéralos a partir de la descripción.'}"
Asignaturas involucradas: "${formData.asignaturas.join(', ')}"
Formato: lista con guiones.`;
    try {
      const ai = new GoogleGenerativeAI(apiKey);
      ...
      setFormData(prev => ({ ...prev, [targetField]: text.replace(/(\*\*|\*)/g,'') as any }));
    } catch (e) {
      console.error(e);
      alert(`Hubo un error al generar ${targetField}.`);
    } finally {
      if (targetField === 'objetivos') setIsGeneratingObjectives(false); else setIsGeneratingIndicators(false);
    }
  };
  ...
  const handleAIGenerateStructure = async () => {
    ...
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
    if (!apiKey) { alert('La API Key de Gemini no está configurada.'); setIsGeneratingStructure(false); return; }
    const getTeacherNames = (ids: string[]) => ids.map(id => (availableTeachers.find(t=>t.id===id)?.nombreCompleto || id)).join(', ');
...
Descripción: "${formData.descripcionProyecto}"
Docentes responsables: "${getTeacherNames(formData.docentesResponsables)}"
Asignaturas: "${formData.asignaturas.join(', ')}"
Estructura:
...
- tareas: 2 a 3 objetos { "instrucciones", "fechaEntrega" (YYYY-MM-DD) }`;
    try {
      const ai = new GoogleGenerativeAI(apiKey);
      const model = ai.getGenerativeModel({ model: 'gemini-1.5-flash-latest'});
      const result = await model.generateContent(prompt);
      ...
      setFormData(prev => ({
        ...
        tareas: structure.tareas?.map((t: any, i: number) => ({ ...t, id: crypto.randomUUID(), numero: (prev.tareas?.length || 0) + i + 1 })) || prev.tareas
      }));
    } catch (e) {
      console.error(e);
      alert('No se pudo generar la estructura del proyecto.');
    } finally {
  ...
  const getTeacherName = (id: string) => availableTeachers.find(t=>t.id===id)?.nombreCompleto || id;
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <div className="space-y-6">
          ...
          <FormCard icon={<Users className="w-6 h-6 text-slate-600" />} label="Profesores Responsables">
            <div className="space-y-3">
              ...
              {!!formData.docentesResponsables.length && (
                <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-1">Profesores Seleccionados:</p>
                  <div className="flex flex-wrap gap-2">
                    {formData.docentesResponsables.map(id => <span key={id} className="px-2 py-1 bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-200 rounded-md text-xs font-medium">{getTeacherName(id)}</span>)}
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="space-y-6">
          ...
          <FormCard icon={<Target className="w-6 h-6 text-slate-600" />} label="Objetivos de Aprendizaje" aiButton={
            <button type="button" onClick={()=>handleAIGeneration('objetivos')} disabled={!formData.descripcionProyecto.trim() || isGeneratingObjectives} className="p-2 rounded-lg bg-purple-100 dark:bg-purple-900/50 hover:bg-purple-200 dark:hover:bg-purple-800/50 disabled:opacity-50 transition-colors" title="Generar con IA">
              <Sparkles className="w-5 h-5 text-purple-600 dark:text-purple-400" />
            </button>
          }>
            {isGeneratingObjectives ? (
              <div className="flex items-center justify-center p-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600"></div><span className="ml-3 text-slate-600 dark:text-slate-400">Generando objetivos...</span></div>
            ) : (
          ...
          <FormCard icon={<BarChart3 className="w-6 h-6 text-slate-600" />} label="Indicadores de Éxito" aiButton={
            <button type="button" onClick={()=>handleAIGeneration('indicadoresLogro')} disabled={!formData.descripcionProyecto.trim() || isGeneratingIndicators} className="p-2 rounded-lg bg-purple-100 dark:bg-purple-900/50 hover:bg-purple-200 dark:hover:bg-purple-800/50 disabled:opacity-50 transition-colors" title="Generar con IA">
              <Sparkles className="w-5 h-5 text-purple-600 dark:text-purple-400" />
            </button>
          }>
            {isGeneratingIndicators ? (
              <div className="flex items-center justify-center p-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600"></div><span className="ml-3 text-slate-600 dark:text-slate-400">Generando indicadores...</span></div>
            ) : (
          ...
          <FormCard icon={<Calendar className="w-6 h-6 text-slate-600" />} label="Planificación y Cronograma">
            <div className="space-y-6">
              <div className="text-center border-b border-slate-200 dark:border-slate-700 pb-6">
                <button type="button" onClick={handleAIGenerateStructure} disabled={isGeneratingStructure || !formData.descripcionProyecto} className="inline-flex items-center gap-3 bg-gradient-to-r from-sky-500 to-blue-600 text-white font-semibold py-3 px-6 rounded-lg disabled:from-slate-400 disabled:to-slate-500 disabled:cursor-not-allowed hover:from-sky-600 hover:to-blue-700 transition-all duration-200 shadow-lg">
                  {isGeneratingStructure ? (<><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div> Generando estructura...</>) : (<><Sparkles className="w-5 h-5" /> Sugerir Estructura Completa con IA</>)} 
                </button>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">La IA generará actividades, fechas clave y tareas basadas en la descripción del proyecto</p>
              </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600">
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
                    <div className="lg:col-span-4">
                      ...
                      <input value={newActivity.nombre} onChange={e=>setNewActivity({...newActivity, nombre: e.target.value})} placeholder="Nombre de la actividad" className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm" />
                    </div>
                    <div className="lg:col-span-3">
                      <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Fecha de Inicio</label>
                      <input type="date" value={newActivity.fechaInicio} onChange={e=>setNewActivity({...newActivity, fechaInicio: e.target.value})} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm" />
                    </div>
                    <div className="lg:col-span-3">
                      <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Fecha de Fin</label>
                      <input type="date" value={newActivity.fechaFin} onChange={e=>setNewActivity({...newActivity, fechaFin: e.target.value})} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm" />
                    </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600">
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
                    <div className="sm:col-span-2">
                      ...
                      <input value={newFechaClave.nombre} onChange={e=>setNewFechaClave({...newFechaClave, nombre: e.target.value})} placeholder="Nombre del hito" className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Fecha</label>
                      <input type="date" value={newFechaClave.fecha} onChange={e=>setNewFechaClave({...newFechaClave, fecha: e.target.value})} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm" />
                    </div>
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600">
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
                    <div className="lg:col-span-6">
                      ...
                      <input value={newTarea.instrucciones} onChange={e=>setNewTarea({...newTarea, instrucciones: e.target.value})} placeholder="Descripción detallada de la tarea" className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm" />
                    </div>
                    <div className="lg:col-span-3">
                      <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Fecha de Entrega</label>
                      <input type="date" value={newTarea.fechaEntrega} onChange={e=>setNewTarea({...newTarea, fechaEntrega: e.target.value})} className="w-full p-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm" />
                    </div>
                    <div className="lg:col-span-3">
                        ...
                        {formData.tareas.map(t => (
                          <div key={t.id} className="p-3 bg-white dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-600">
                            <div className="flex justify-between items-start">
                              ...
                              <span className="text-slate-500 dark:text-slate-400 text-xs ml-3">{t.fechaEntrega}</span>
                            </div>
                          </div>
                        ))}
                      </div>
...
const Interdisciplinario: React.FC = () => {
  ...
  const handleSave = async (plan: Omit<PlanificacionExtendida, 'id'> | PlanificacionExtendida) => {
    try {
      const planToSave: any = { ...plan, docentesResponsables: Array.isArray((plan as any).docentesResponsables) ? (plan as any).docentesResponsables.join(', ') : (plan as any).docentesResponsables };
      if ('id' in plan && (plan as any).id) await updatePlanificacion((plan as any).id, planToSave);
      else await createPlanificacion(planToSave);
  ...
  const handleExportProjectPDF = async (plan: PlanificacionExtendida) => {
    ...
    if (plan.tareas && plan.tareas.length > 0) {
      const tasksBody = plan.tareas.map(t => [String(t.numero || ''), t.instrucciones, t.fechaEntrega]);
      ...
      autoTable(doc, { startY: y, head: [['#', 'Instrucciones', 'Fecha de Entrega']], body: tasksBody, theme: 'striped', headStyles: { fillColor: [52,73,94] } });
      // @ts-ignore
      y = (doc as any).lastAutoTable.finalY + 15;
    }
    ...
    if (timelineElement) {
      try {
        const canvas = await html2canvas(timelineElement as HTMLElement, { scale: 2, backgroundColor: '#ffffff' });
        const imgData = canvas.toDataURL('image/png');
        doc.addPage(); addPageHeader(); y = margin + 15;
        doc.setFont('helvetica', 'bold'); doc.setFontSize(FONT_HEADER); doc.setTextColor(40); doc.text('Cronograma del Proyecto', margin, y); y += 10;
        const imgProps: any = doc.getImageProperties(imgData); const imgWidth = contentWidth; const imgHeight = (imgProps.height * imgWidth) / imgProps.width;
        doc.addImage(imgData, 'PNG', margin, y, imgWidth, imgHeight);
      } catch (e) { console.error('Error generating timeline image:', e); }
    }

    const pageCount = (doc.internal as any).pages.length - 1;
    for (let i=1;i<=pageCount;i++){ doc.setPage(i); doc.setFont('helvetica','normal'); doc.setFontSize(9); doc.setTextColor(150); doc.text(`Página ${i} de ${pageCount}`, pageWidth - margin, doc.internal.pageSize.getHeight() - 10, { align: 'right' }); }
  ...
  if (view === 'dashboard') {
    return <Dashboard planificaciones={planificaciones} onBack={()=>setView('list')} />;
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <div className="flex items-center gap-4">
              <button onClick={()=>setView('dashboard')} className="inline-flex items-center gap-3 bg-gradient-to-r from-green-600 to-teal-600 text-white font-bold py-3 px-6 rounded-lg hover:from-green-700 hover:to-teal-700 transition-all duration-200 shadow-lg">
                <TrendingUp className="w-5 h-5" /> Ver Dashboard
              </button>
              <button onClick={handleCreateNew} className="inline-flex items-center gap-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold py-3 px-6 rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-lg">
          ...
          {planificaciones.length === 0 ? (
            <div className="text-center py-16">
              ...
              <button onClick={handleCreateNew} className="inline-flex items-center gap-2 bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors">
              ...
              {planificaciones.map(plan => (
                <div key={plan.id} className="group bg-slate-50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl p-6 hover:shadow-lg transition-all duration-200 hover:border-blue-300 dark:hover:border-blue-600">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-600">
                    <div className="flex items-center gap-2">
                      <button onClick={()=>setViewingSubmissionsForPlan(plan)} className="inline-flex items-center gap-1 text-green-600 hover:text-green-700 dark:text-green-400 dark:hover:text-green-300 font-semibold text-sm transition-colors" title="Ver entregas de estudiantes">
                        <CheckCircle className="w-4 h-4" /> Entregas
                      </button>
                      <button onClick={()=>handleExportProjectPDF(plan)} disabled={isExportingId===plan.id} className="inline-flex items-center gap-1 text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 font-semibold text-sm transition-colors disabled:opacity-50" title="Exportar a PDF">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={()=>handleEdit(plan)} className="p-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg transition-colors" title="Editar proyecto">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={()=>handleDelete(plan.id)} className="p-2 text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-lg transition-colors" title="Eliminar proyecto">
      ...
      {planificaciones.length > 0 && view === 'list' && (
        <div id="gantt-container-vertical">
          <ProjectTimeline planificaciones={planificaciones} onActivityUpdate={handleActivityUpdate} />
        </div>
      )}
    </div>
  );
