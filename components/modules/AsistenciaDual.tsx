...
import {
    getAllAsistenciaRecords,
    getAsistenciaByMonth,
} from '../../src/firebaseHelpers/asistenciaDual'; // AJUSTA la ruta según dónde guardes los helpers
import {
    getAllUsers,
} from '../../src/firebaseHelpers/users'; // Helper de usuarios existente
...
const AsistenciaDual: React.FC = () => {
    ...
    const loadData = useCallback(async () => {
        setLoading(true);
        try {
            // Cargar registros de asistencia del mes actual (optimización)
            const year = currentDate.getFullYear();
            const month = currentDate.getMonth();
            const asistenciaRecords = await getAsistenciaByMonth(year, month);
            setAllRegistros(asistenciaRecords);
            ...
            setError("No se pudieron cargar los datos de asistencia desde la nube.");
        } finally {
            setLoading(false);
        }
    }, [currentDate]);
    ...
    return (
        <div className="bg-white p-6 md:p-8 rounded-xl shadow-md space-y-6 animate-fade-in">
            ...
            {loading && <div className="text-center text-amber-600 py-4">Cargando datos de asistencia desde la nube...</div>}
            {error && <div className="text-red-600 bg-red-100 p-3 rounded-md mb-4">{error}</div>}
            
            {!loading && allRegistros.length === 0 && (
                <div className="bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700 p-4 rounded-lg">
                    No hay registros de asistencia para este mes en la nube.
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border rounded-lg bg-slate-50 dark:bg-slate-700/50 dark:border-slate-700">
                <div className="flex items-center justify-center md:justify-start gap-2">
                    <button 
                        ...
                        disabled={loading}
                        className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        &lt;
                    </button>
                 ...
                 <select 
                    ...
                    disabled={loading}
                    className="w-full border-slate-300 rounded-md shadow-sm dark:bg-slate-700 dark:border-slate-600 disabled:opacity-50"
                >
                            ...
                            {studentsForCourse.length > 0 ? studentsForCourse.map(student => (
                                ...
                                <tr>
                                    <td colSpan={monthDays + 1} className="text-center p-8 text-slate-500">
                                        {loading ? 'Cargando...' : 'No hay estudiantes en el curso seleccionado o que coincidan con la búsqueda.'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
